...
**框架（Frame）**:
模板中固定照搬的部分：母版品牌层的几何与图像——顶部绿条、左下 logo、右下页码、封面三 logo 版式。只锁品牌元素的形状与位置；一切字体、字号、颜色的决策都不属于框架层，归内容形式。
...
**内容形式（Content form）**:
框架之外的全部样式决策域：一切文字的字体字号字重（含绿条内标题）、层级策略、间距节奏、配色（含强调色）、图表规范、图文排布。归本项目自行设计，永不从模板作者的习惯继承。
